...
class Junkyard
  ...
  def create_deck
    ...
    3.times do
      ...
      @deck << Card.new(channel, :grease_bucket)
...
class JunkyardPlugin < Plugin
  ...
  def add_bot(m, plugin)
    ...
    unless @games.key?(m.channel)
      @games[m.channel] = Junkyard.new(self, m.channel, m.source, title(m))
    end
    g = @games[m.channel]
  ...
  def stop_game(m, plugin=nil)
    ...
    if manager == player or @bot.auth.irc_to_botuser(m.source).owner?
      ...
      @bot.say m.channel, "#{title(m)} stopped."
    else
  ...
  def show_stats(m, params)
    ...
    unless @registry[x][2].has_key? y
      @bot.say m.replyto, "#{y} hasn't played #{title(m)} in this channel."
      return
    end
